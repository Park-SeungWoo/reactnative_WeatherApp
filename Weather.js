        ...
        <View style={styles.maininfobox}>
          <View style={styles.rowInfo}>
            <View style={styles.InfoItems}>
              ...
              <Text style={styles.scrollboxtext}>
                {Math.floor(infos.current.feels_like - 273.15)}°
              </Text>
            </View>
            <View style={styles.InfoItems}>
          ...
          <View style={styles.rowInfo}>
            <View style={styles.InfoItems}>
              ...
              <Text style={styles.scrollboxtext}>
                {Math.floor(infos.daily[0].temp.max - 273.15)}°
              </Text>
            </View>
            <View style={styles.InfoItems}>
              ...
              <Text style={styles.scrollboxtext}>
                {Math.floor(infos.daily[0].temp.min - 273.15)}°
              </Text>
            </View>
          </View>
        </View>
        <ScrollView
          style={styles.hourlyinfoscroll}
          horizontal={true}
          showsHorizontalScrollIndicator={false}
        >
          <View style={styles.hourlyinfoview}>
            {infos.hourly.map((info, i) =>
              i < 25 ? (
                i == 0 ? (
                  <View style={styles.hourlyinfobox}>
                    <Text style={styles.hourlyinfotext}>{hour}시</Text>
                    <MaterialCommunityIcons
                      name={WeatherCases[info.weather[0].main].icon}
                      color={'white'}
                      size={20}
                    />
            ...
            i == 0 ? (
              <View style={styles.dailyinfobox}>
                ...
                <View style={styles.dailyinfoiconview}>
                  <MaterialCommunityIcons
                    name={WeatherCases[info.weather[0].main].icon}
                    style={styles.dailyinfoicon}
                  />
                </View>
                <Text style={styles.dailyinfotemp}>
                  {Math.floor(info.temp.max - 273.15)}° {Math.floor(info.temp.min - 273.15)}°
                </Text>
              </View>
            ) : (
              <View style={styles.dailyinfobox}>
                <Text style={styles.dailyinfotext}>
                  {DateCases[Number(date) + i < 7 ? Number(date) + i : Number(date) + i - 7]}
                </Text>
                <View style={styles.dailyinfoiconview}>
                  <MaterialCommunityIcons
                    name={WeatherCases[info.weather[0].main].icon}
